...
SELECT send AS batch_msg_id3 from pgmq.send('test_batch_set_vt_queue', '0') \gset

-- set multiple messages invisible for 100 seconds
SELECT ARRAY(
...
-- read messages, first two should not be visible
SELECT msg_id = :batch_msg_id3 from pgmq.read('test_batch_set_vt_queue', 1, 5);
...
) = ARRAY[:batch_msg_id1, :batch_msg_id2, :batch_msg_id3]::bigint[];
